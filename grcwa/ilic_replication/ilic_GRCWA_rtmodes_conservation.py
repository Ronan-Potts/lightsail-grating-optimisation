...
Ris = np.empty((len(thetas), len(orders)-1))
Tis = np.empty((len(thetas), len(orders)-1))
Ris_plus_Tis = np.empty((len(thetas), 1))
...
for j in range(0,len(thetas)):
    ...
    '''
    # Ri(Ti) has length obj.nG for reflectance (transmittance) at various orders, to see which order, check obj.G; too see which kx,ky, check obj.kx obj.ky
    ...
    Ris[j,:] = Rs
    Tis[j,:] = Ts
    # Sum of all reflection and transmission
    Ris_plus_Tis[j,:] = sum(R_plus_Ts)
...
for i in range(0,len(orders)):
    ...
    if type(order) == int:
        ...
        plt.plot(thetas*180/np.pi, Ris_plus_Tis[:,], color=color)
        legend.append(r'$R+T$')
